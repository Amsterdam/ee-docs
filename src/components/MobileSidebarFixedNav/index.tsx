...
const MobileSidebarFixedNav: FunctionComponent = () => {
  ...
  return (
    <div className={styles.container}>
      ...
      >
        <><GitHubIcon width="24" height="24" className={isLightMode ? '' : styles.invert} /><span>GitHub</span></>
      </a>
      <button
        className={`menu__link menu__link--sublist ${styles.button}`}
        onClick={() => setColorMode(isLightMode ? 'dark' : 'light')}
      >
        {isLightMode ? <><IconDarkMode /><span>Toggle dark mode</span></> : <><IconLightMode /><span>Toggle light mode</span></>}
      </button>
    </div>
  );
};
