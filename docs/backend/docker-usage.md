...
- This page was last reviewed 1st october 2024. It needs to be reviewed again on the 1st April 2025.
...
- [ ] **Base Image**: Use well-known and well-maintained base images such as `python:3.9-slim` or `node:20-alpine`, optimized for security and performance. A link to an approved list of base images can be added, or a list of approved base images can be shown here.
...
- **Docker Image Storage**: Dockerfiles must be stored in the application repository in **GitHub**. The compiled Docker images must be stored in **Azure Container Registry (ACR)**. Only in cases where an image is shared across multiple teams or other municipalities, **Docker Hub** may be used.
...
- **Regular Updates**: Docker images must be updated regularly to apply security patches and performance improvements. A minimum update frequency should be established, for example, monthly or quarterly, to provide a clear framework for what is meant by "regular."

## What to avoid?

- Avoid hard-coding sensitive information such as passwords in Dockerfiles.
- Do not use unofficial or uncertified Docker images.
- Avoid using outdated Docker images. Ensure that images are regularly updated to minimize security risks.
- **Best practice**: Use a single artifact that is deployed across all environments (development, testing, acceptance, and production). Avoid building different images for D, T, A, and P.
